...
package bsu.rfe.java.course2.group6.Kupriyanov;
...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() { //констуктор
    ...
    //Инициализирует данные для графика и вычисляет минимальные и максимальные значения по осям X и Y
    ...
    //Масштабирует отображение графика на заданную область
    ...
    //Основной метод для отрисовки, вызывает методы для отрисовки сетки, осей, графика, маркеров, подписей
    ...
    //Отображает линии, соединяющие точки данных на графике
    ...
    //Рисует маркеры на графике и подсвечивает данный маркер
    ...
    //Добавляет подписи к осям и маркерам, показывая значения X и Y
    ...
    //Рисует оси координат (X,Y) и добавляет их подписи
    ...
    //Преобразует координаты (x,y) из логической системы координат в экранные координаты
    ...
    //Обратное преобразование - переводит экранные координаты в логические координаты
    protected double[] translatePointToXY(int x, int y) {
        return new double[]{this.viewport[0][0] + (double)x / this.scaleX, this.viewport[0][1] - (double)y / this.scaleY};
    }

    //Ищет ближайшую точку на графике с экранными координатами (x, y)
    ...
    //Сбрасывает график к исходным данным
    public void reset() {
        this.displayGraphics(this.originalData);
    }

    //Обрабатывает клики и нажатия мыши
    public class MouseHandler extends MouseAdapter {
        public MouseHandler() {
        }

        //Правая кнопка мыши: Отменяет последнее масштабирование. Если истории отмен нет, возвращает к исходному масштабу.
        ...
        //Левая кнопка мыши: Определяет, была ли выбрана точка.
        ...
        // Завершает перемещение точки или масштабирование
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        //Проверяет, наведена ли мышь на точку графика
        ...
        //Обновляет координаты выбранной точки
